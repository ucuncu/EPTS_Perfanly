...
uint32_t Offset = 0;              /* R/W Offset */
uint32_t Length = 0;                  /* R/W Length */

uint8_t  BulkStage = 0;               /* Bulk Stage */
...
void MSC_Inquiry (void) {
  ...
  BulkBuf[ 8] = 'E';                       /* Vendor Identification */
  BulkBuf[ 9] = 'P';
  BulkBuf[10] = 'T';
  BulkBuf[11] = 'S';
  BulkBuf[12] = 'P';
...
fail: 
    ...
    /* Invalid CBW */
    MSC_SetStallEP(MSC_EP_IN);                                         /* set EP to stay stalled */
    USB_EndPointStall |=  (MSC_EP_IN  & 0x80) ? ((1 << 16) << (MSC_EP_IN  & 0x0F)) : (1U << MSC_EP_IN);
    MSC_SetStallEP(MSC_EP_OUT);                                         /* set EP to stay stalled */
