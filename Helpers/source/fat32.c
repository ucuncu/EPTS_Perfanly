            ...
            if(fileOperation == READ)
            {
                *fileOpenSts = openFile( READ, filePtr);
                if(*fileOpenSts != FAT32_FILE_OPEN_SUCCESSFUL)
                {
								if (fEnableDebugLog)
								{
									printf("FILE_DELETED_OR_NOT_FOUND..\n");
								}    
                    ...
                    if(filePtr->cluster == 0)
                    {
                       ...
                       *fileOpenSts = FAT32_NO_FREE_CLUSTERS_FOUND;
                       return 0;
										...
										if (fEnableDebugLog)
										{
											printf("get/Set NextCluster..\n");   //last cluster of the file, marked END_OF_CLUSTERS
										}
...
uint8_t FILE_Delete(char *fileName)
{
    ...
    if(returnStatus == FAT32_VALID_FILE_NAME)
    {
			if (fEnableDebugLog)
			{
				printf("Searching for Opened File..\n");
			}
...
void FILE_PutCh (char data)
{
    ...
    {
        if((filePtr->endOfFileDetected == 0) && (data != SD_EOF))
        {
        ...
        else if(filePtr->endOfFileDetected == 0)
        {
            ...
            while(1)
            {
                ...
                if(filePtr->cluster == 0)
                {
									if (fEnableDebugLog)
									{
										printf("Error in getting cluster..\n");
									}
                  return;
                }
