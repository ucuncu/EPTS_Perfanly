...
volatile int8_t I2CSlaveBuffer[I2C_PORT_NUM][BUFSIZE];
...
uint32_t I2C_Read_One_Byte( uint32_t portNum, uint32_t slaveAdress, uint32_t registerAdress );
uint32_t I2C_Write_One_Byte( uint32_t portNum, uint32_t slaveAdress, uint32_t registerAdress, uint8_t data );
...
/*****************************************************************************
** Function name:		I2C_Write_One_Byte
**
** Descriptions:		This Rountine Writes one Byte to Slave Address Register 
**
** parameters:			I2C port number  - 0,1 or 2
** slave addres:    8 bit slave address 
** register address: 8 bit slave address
** register address: 8 bit data to write
** Returned value:		master state of current I2C port. (0 to 12)
** 
*****************************************************************************/
uint32_t I2C_Write_One_Byte( uint32_t portNum, uint32_t slaveAdress, uint32_t registerAdress, uint8_t data )
{
	uint32_t ret;
	I2CWriteLength[portNum] = 3;
  I2CReadLength[portNum] = 0;
  I2CMasterBuffer[portNum][0] = slaveAdress;
  I2CMasterBuffer[portNum][1] = registerAdress;		/* address */
  I2CMasterBuffer[portNum][2] = data;
  ret = I2CEngine( portNum );
	return ret;
}
...
{
	uint32_t ret;
	I2CWriteLength[portNum] = 2;
  I2CReadLength[portNum] = 1;
  I2CMasterBuffer[portNum][0] = slaveAdress;
  I2CMasterBuffer[portNum][1] = registerAdress;		/* address */
  I2CMasterBuffer[portNum][2] = slaveAdress | RD_BIT;
  ret = I2CEngine( portNum );
	return ret;
}

/***********************************************************************
 * @brief		I2C bus end of communication status
 ***********************************************************************/
const char *returnSensorError( uint8_t status)	
{
	uint8_t sensorStatus = status;
	if (sensorStatus == 0)
	{
		return "I2C_IDLE" ;
	}
	...
	else if (sensorStatus == 9)
	{
		return "I2C_NACK_ON_DATA" ;		
	}
	else if (sensorStatus == 10)
	{
		return "I2C_ARBITRATION_LOST" ;		
	}
	else if (sensorStatus == 11)
	{
		return "I2C_TIME_OUT" ;		
	}
	else if (sensorStatus == 12)
	{
		return "I2C_OK" ;		
	}
	else
